    ...
    return (
        ...
        >
            <span className="pointer-events-none bg-linear-to-b from-black to-gray-300/80 bg-clip-text text-center text-8xl leading-none font-semibold whitespace-pre-wrap text-transparent dark:from-white dark:to-slate-900/10">
                {/* UI:DOCK_TITLE (The large prominent title) */}
                {UI.headerText}
            </span>
            <TooltipProvider>
                <Dock direction="middle">
                    {UI.navbar.map((item) => (
                        <DockIcon key={item.label}>
                            <Tooltip>
                                ...
                                <TooltipContent>
                                    <p>
                                        {/* UI:NAV_LABEL (The hover tooltip for navigation links) */}
                                        {item.label}
                                    </p>
                                </TooltipContent>
                            </Tooltip>
                    ...
                    <Separator orientation="vertical" className="h-full" style={{ backgroundColor: COLORS.secondary }} />
                    {Object.entries(UI.contact.social).map(([name, social]) => (
                        <DockIcon key={name}>
                            <Tooltip>
                                <TooltipTrigger
                                    render={
                                ...
                                <TooltipContent>
                                    <p>
                                        {/* UI:SOCIAL_NAME (The hover tooltip for social links) */}
                                        {social.name}
                                    </p>
                                </TooltipContent>
                            </Tooltip>
